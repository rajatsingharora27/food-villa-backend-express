  ...
  addProduct = async (inputRequest: C_W_ProdcutAdd, refId: string) => {
    try {
      if (process.env.JWT_SECRET !== undefined) {
        const decode = jwt.verify(inputRequest.token, process.env.JWT_SECRET) as JwtTokenType;
        if (decode) {
      ...
      log.error(`Exception occurred in {{addProduct()}} refId${refId} , ex:${ex}`);
    }
  };

  private async addUserWishListProduct(tokenDecodedValue: JwtTokenType, inputRequest: C_W_ProdcutAdd, refId: string) {
    const userWishlistDetails = await userWishListModel.findOne({ email: tokenDecodedValue.data.email });
    if (userWishlistDetails != null) {
      ...
      const listIfItemIsAlreadyThere = currentWishListItemsUserHave.filter((item: any) => item.product != inputRequest.productId);
      if (listIfItemIsAlreadyThere.length !== currentWishListItemsUserHave.length) {
      ...
      await userWishlistDetails.updateOne({ wishlistItem: listOfItemsToBeAddedInWishList });
      log.info(`Product added to the wishlist  productId:${inputRequest.productId} , refId:${refId}`);
    }
  }

  private async addToUserCartProduct(tokenDecodedValue: JwtTokenType, inputRequest: C_W_ProdcutAdd, refId: string) {
    ...
    try {
      const isUserInCartTable = await cartUserModel.findOne({ email: tokenDecodedValue.data.email });
      let cartList;
      ...
      if (isUserInCartTable === null) {
        ...
        const produtListUserAlreadyHasInCart = isUserInCartTable.cartItem.filter((ele) => ele.product == inputRequest.productId);

        if (isUserInCartTable.cartItem.length !== produtListUserAlreadyHasInCart.length) {
      ...
      log.error(`exception occurred in {{addToUserCartProduct()}}  refid:${refId}  , ex : ${ex}`);
    }
  }

  private getCartList(inputRequest: C_W_ProdcutAdd, prodcutEarlierQuantityInCart = 0) {
    let userCartItem = [];
    let quantity;
    if (inputRequest.cartOrWishlist.toLowerCase() === "c") {
      if ((inputRequest.productToCartQuantity.decrease == true && inputRequest.productToCartQuantity.increase == true) || (inputRequest.productToCartQuantity.decrease === false && inputRequest.productToCartQuantity.increase == false)) {
        ...
        if (inputRequest.productToCartQuantity.increase === true) {
          quantity = inputRequest.productToCartQuantity.quantity + prodcutEarlierQuantityInCart;
        } else if (inputRequest.productToCartQuantity.decrease === true) {
          if (prodcutEarlierQuantityInCart > inputRequest.productToCartQuantity.quantity) {
            quantity = prodcutEarlierQuantityInCart - inputRequest.productToCartQuantity.quantity;
          } else {
            quantity = inputRequest.productToCartQuantity.quantity - prodcutEarlierQuantityInCart;
          }
